...
int main(int argc, char **argv)
{
	...
	if(STANDARD)
	{
		...
		compute_ders(Ix,Iy,It,floatpic,pic_t,pic_x,pic_y,offset);
...
void convolve_Gaussian(unsigned char inpic[PIC_T][PIC_X][PIC_Y],float floatpic[FIVE][PIC_X][PIC_Y],unsigned char pic[FIVE][PIC_X][PIC_Y],float sigma,int pic_t,int pic_x,int pic_y,int start,int frame,int time)
{
	...
	if(sigma != 0.0){
	#pragma omp parallel for \
		private(i) shared(mask,offset,sum)\
		schedule (static)
		for(i=0;i<size;i++)
		{
	...
	//int pic_x,pic_y;
{
	...
	n = 0;
#pragma omp parallel for \
	private(i,j,t1,t2) shared(sum,n) \
	schedule(static)
	for(i=0;i<pic_x;i++){
		for(j=0;j<pic_y;j++)
		{
...
#pragma omp parallel for \
	private(i,j) shared(gradient2,tau2,count) \
	schedule (static)
	for(i=0;i<pic_x;i++){
		for(j=0;j<pic_y;j++)
		{
